...
class Api {
    ...
    public static function orderQuery($inputObj, $timeOut = 6)
    {
        ...
        $startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($xml, $url, false, $timeOut);
        $result = Results::Init($response, true);
        self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
    ...
    public static function transfers($inputObj, $timeOut = 6)
    {
        ...
        //检测必填参数
        if (!$inputObj->GetKeyVal('partner_trade_no')) {
            throw new Exception("企业向微信用户个人付款接口，partner_trade_no 为必填参数");
        }

        if (!$inputObj->GetOpenid()) {
            throw new Exception("企业向微信用户个人付款接口，openid为必填参数");
        }

        if (!$inputObj->GetKeyVal('check_name')) {
            throw new Exception("企业向微信用户个人付款接口，校验用户姓名选项为必填参数");
        }
        if ($inputObj->GetKeyVal('amount')  == 0) {
            throw new Exception("企业向微信用户个人付款接口，金额为必填参数并大于0");
        }

        if (strlen($inputObj->GetKeyVal('desc')) == 0) {
            throw new Exception("企业向微信用户个人付款接口，企业付款备注为必填参数");
        }

        if (strlen($inputObj->GetKeyVal('spbill_create_ip')) == 0) {
            throw new Exception("企业向微信用户个人付款接口，Ip地址为必填参数");
        }
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
     ...
     * @throws \ddliu\wxpay\Exception
     */
    public static function gettransferinfo($inputObj, $timeOut = 6)
    {
    ...
    public static function report($inputObj, $timeOut = 1)
    {
        ...
        $inputObj->SetAppid(Config::$APPID);//公众账号ID
        $inputObj->SetMch_id(Config::$MCHID);//商户号
        $inputObj->SetUser_ip(self::getIp());//终端ip
    ...
    public static function notifyData() {
        //获取通知的数据
        $xml = file_get_contents('php://input');
        if (!$xml) return false;
        //如果返回成功则验证签名
        try {
            $data = Results::Init($xml);
            return $data;
        } catch (Exception $e){
            $msg = $e->errorMessage();
            return false;
