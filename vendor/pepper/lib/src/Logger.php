...
class Logger
{
    ...
    public static function getInstance()
    {
        if( self::$instance === null )
        {
            ...
            self::$instance = new Logger(SimpleConfig::get('LOG'), $intStartTime);
        }
	...
	public function writeLog($intLevel, $str, $errno = 0, $arrArgs = null, $depth = 0)
    {
        ...
        $str = sprintf( "%s: time[%s] [%s:%d] errno[%d] ip[%s] logId[%u] uri[%s] time_used[%d] %s%s\n",
                        $strLevel,
                        date('m-d H:i:s', time()),
                        $file, $line, $errno,
                        self::getClientIP(),
