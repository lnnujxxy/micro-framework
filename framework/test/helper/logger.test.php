...
class TestHttp extends UnitTestCase {
	...
	function testLog() {
		$this->logObj->log("error", Helper_Log::LEVEL_ERROR);
		$this->logObj->log("warning", Helper_Log::LEVEL_WARNING);
	...
	function testFilterLogLevel() {
		$this->logObj->log("error", Helper_Log::LEVEL_ERROR);
		$this->logObj->log("warning", Helper_Log::LEVEL_WARNING);
		$this->logObj->log("info", Helper_Log::LEVEL_INFO);
		$this->logObj->log("profile", Helper_Log::LEVEL_PROFILE);
	...
	function testFilterLogCategories() {
		$this->logObj->log("error", Helper_Log::LEVEL_ERROR, 'cat1');
		$this->logObj->log("warning", Helper_Log::LEVEL_WARNING, 'cat2');
		$this->logObj->log("info", Helper_Log::LEVEL_INFO, 'cat3');
		$this->logObj->log("profile", Helper_Log::LEVEL_PROFILE, 'cat4');
	...
	function setUp() {
		$this->logObj = Helper_Log::factory('EP_FileLog', dirname(__FILE__).'/');
	}
}